...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message:
        'O token de acesso salvo não pôde ser descriptografado — provavelmente a ENCRYPTION_KEY mudou. Insira o token novamente para corrigir.',
    })
  }
  ...
  if (config.waba_id) {
    try {
      ...
      if (!checks.waba_subscribed_to_app) {
        errors.push(
          'O WABA não tem apps inscritos. Salve a configuração novamente para se inscrever.',
        )
      }
    } catch (err) {
      errors.push(
