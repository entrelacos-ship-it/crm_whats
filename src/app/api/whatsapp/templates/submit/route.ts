  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Modelos de AUTHENTICATION ainda não são suportados aqui — crie-os no Meta WhatsApp Manager e use "Sincronizar da Meta".',
        },
        { status: 400 },
      )
    }
    ...
    if (dryRun) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: e instanceof Error ? e.message : 'Falha no upload da imagem de cabeçalho.' },
          { status: 400 },
        )
      }
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: isRateLimit
              ? 'Limite de requisições da Meta atingido (100 criações de modelo por hora). Tente novamente mais tarde.'
              : message,
          },
    ...
    if (upsertErr) {
      ...
      return NextResponse.json(
        {
          error: `Enviado à Meta, mas falhou ao salvar localmente: ${upsertErr.message}. Execute "Sincronizar da Meta" para recuperar.`,
