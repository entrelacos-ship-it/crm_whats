  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error:
            'Modelos de AUTHENTICATION não são editáveis aqui — gerencie-os no Meta WhatsApp Manager.',
        },
        { status: 400 },
      )
    }
    ...
    if (updErr) {
      return NextResponse.json(
        {
          error: `Editado na Meta, mas falhou ao salvar localmente: ${updErr.message}. Execute "Sincronizar da Meta" para recuperar.`,
        },
        { status: 500 },
      )
    }
