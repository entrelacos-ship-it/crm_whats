  ...
  try {
    ...
    if (Array.isArray(newRecipients) && newRecipients.length > 0) {
      ...
      return NextResponse.json(
        {
          error:
            'Informe `recipients` (preferido) ou `phone_numbers` — deve ser um array não vazio',
        },
        { status: 400 }
      )
    }
    ...
    if (rawTemplateRow && !isMessageTemplate(rawTemplateRow)) {
      return NextResponse.json(
        {
          error:
            'O registro do template está malformado localmente — execute "Sincronizar com a Meta" nas Configurações para corrigi-lo antes de enviar.',
        },
        { status: 500 },
      )
    }
    ...
    return NextResponse.json(
      { error: 'Falha ao processar o envio em massa' },
      { status: 500 }
    )
  }
}
