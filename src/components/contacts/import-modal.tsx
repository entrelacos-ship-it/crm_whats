...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Confirme que o CSV tem uma coluna "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      try {
        ...
        toast.warning('Contatos importados, mas algumas tags falharam ao aplicar.');
      }
            ...
            <div className="space-y-3">
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + {parsedRows.length - PREVIEW_LIMIT} outra
                  {parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} linha
